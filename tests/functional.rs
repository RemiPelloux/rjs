...
impl Drop for TestEnv {
    fn drop(&mut self) {
        ...
        std::env::set_current_dir(&self.original_dir).expect("Failed to change back to original directory");
...
fn test_init_command() {
    ...
    assert!(stdout.contains("Created package.json"), "Init output doesn't contain expected message");
    ...
    let json: serde_json::Value = serde_json::from_str(&content).expect("Failed to parse package.json");
    
    assert!(json.get("name").is_some(), "package.json is missing 'name' field");
    assert!(json.get("version").is_some(), "package.json is missing 'version' field");
    assert!(json.get("dependencies").is_some(), "package.json is missing 'dependencies' field");
}
...
fn test_install_command() {
    ...
    assert!(stdout.contains("Installing specified packages"), "Install output doesn't contain expected message");
    assert!(stdout.contains("Installed"), "Install output doesn't show that package was installed");
...
fn test_list_command() {
    ...
    assert!(list_empty_stdout.contains("No dependencies found") || list_empty_stdout.contains("0 dependencies"), 
