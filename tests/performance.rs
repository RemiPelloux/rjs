...
fn test_command_performance() {
    ...
    // Test init command
    let (init_duration, init_stdout, init_success) = 
        measure_command(&["init", "-y"]);
    print_result("init -y", init_duration, init_success, &init_stdout);
    
    // Test list command when empty
    let (list_empty_duration, list_empty_stdout, list_empty_success) = 
        measure_command(&["list"]);
    print_result("list (empty)", list_empty_duration, list_empty_success, &list_empty_stdout);
    ...
    print_result("install lodash", install_single_duration, install_single_success, &install_single_stdout);
    
    // Test list command after installation
    let (list_after_duration, list_after_stdout, list_after_success) = 
        measure_command(&["list"]);
    print_result("list (after install)", list_after_duration, list_after_success, &list_after_stdout);
    ...
    print_result("install multiple packages", install_multi_duration, install_multi_success, &install_multi_stdout);
    ...
    print_result("install with --save-dev", install_dev_duration, install_dev_success, &install_dev_stdout);
    
    // Test list with filters
    let (list_dev_duration, list_dev_stdout, list_dev_success) = 
        measure_command(&["list", "--dev"]);
    print_result("list --dev", list_dev_duration, list_dev_success, &list_dev_stdout);
    
    let (list_prod_duration, list_prod_stdout, list_prod_success) = 
        measure_command(&["list", "--production"]);
    print_result("list --production", list_prod_duration, list_prod_success, &list_prod_stdout);
    
    // Print summary of performance
    println!("\n=== Performance Summary ===");
    println!("init -y: {:.4} seconds", init_duration.as_secs_f64());
    println!("list (empty): {:.4} seconds", list_empty_duration.as_secs_f64());
    println!("install lodash: {:.4} seconds", install_single_duration.as_secs_f64());
    println!("list (after install): {:.4} seconds", list_after_duration.as_secs_f64());
    println!("install multiple: {:.4} seconds", install_multi_duration.as_secs_f64());
    println!("install --save-dev: {:.4} seconds", install_dev_duration.as_secs_f64());
    println!("list --dev: {:.4} seconds", list_dev_duration.as_secs_f64());
    println!("list --production: {:.4} seconds", list_prod_duration.as_secs_f64());
    
    // Get total test time
    let total_time = init_duration + list_empty_duration + install_single_duration + 
                    list_after_duration + install_multi_duration + install_dev_duration + 
                    list_dev_duration + list_prod_duration;
