...
impl NpmRegistry {
    ...
    pub async fn get_package_info(&self, package_name: &str) -> Result<PackageInfo> {
        ...
        let npm_package: NpmPackageResponse = response.json().await
            .with_context(|| format!("Failed to parse package info for {}", package_name))?;
            
        // Convert to our internal model
        let versions = npm_package.versions.into_iter()
    ...
    pub async fn download_package(&self, tarball_url: &str, output_path: &std::path::Path) -> Result<()> {
        debug!("Downloading package from {}", tarball_url);
        
        let response = self.client.get(tarball_url)
            .send()
            .await
    ...
    pub fn extract_tarball(&self, tarball_path: &std::path::Path, output_dir: &std::path::Path) -> Result<()> {
        debug!("Extracting tarball {} to {}", tarball_path.display(), output_dir.display());
