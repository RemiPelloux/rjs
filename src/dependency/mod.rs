...
impl DependencyResolver {
    ...
    pub async fn resolve_package(&mut self, name: &str, version_req: &str) -> Result<Package> {
        ...
        let best_version = package_info.versions.keys()
            .filter_map(|v| {
        ...
        debug!("Selected version {} for {}@{}", best_version, name, version_req);
    ...
    pub async fn install_tree(&self, tree: &DependencyTree, install_path: &Path) -> Result<()> {
        ...
        for (_, pkg) in &tree.dependencies {
            ...
            debug!("Installing {} {} to {}", pkg.name, pkg.version, pkg_dir.display());
...
pub fn read_package_json(path: &Path) -> Result<Package> {
    ...
    let dev_dependencies = json.get("devDependencies")
        .and_then(|deps| deps.as_object())
        .map(|obj| {
            obj.iter()
                .filter_map(|(k, v)| {
...
pub fn update_package_json(path: &Path, dependencies: &HashMap<String, String>, dev: bool) -> Result<()> {
    let content = std::fs::read_to_string(path)?;
    let mut json: serde_json::Value = serde_json::from_str(&content)?;
    
    let deps_field = if dev { "devDependencies" } else { "dependencies" };
    
    let deps_obj = json.get_mut(deps_field)
        .and_then(|v| v.as_object_mut())
        .unwrap_or_else(|| {
            json[deps_field] = serde_json::Value::Object(serde_json::Map::new());
            json[deps_field].as_object_mut().unwrap()
        });
